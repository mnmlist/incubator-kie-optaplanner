...
** Main class: `com.sankuai.optaplanner.examples.app.OptaPlannerExamplesApp`
...
The OptaPlanner jars are available in http://search.maven.org/#search|ga|1|com.sankuai.optaplanner[the central maven repository]
(and the snapshots in pass:macros[https://repository.jboss.org/nexus/index.html#nexus-search;gav~com.sankuai.optaplanner~~~~[the JBoss maven repository\]]).
...
----
dependencies {
  implementation 'com.sankuai.optaplanner:optaplanner-core:#{site.pom.latestFinal.version}'
}
----
