...
In a typical project (following the Maven directory structure), that DRL file would be located at `$PROJECT_DIR/src/main/resources/com/sankuai/optaplanner/examples/nqueens/solver/nQueensConstraints.drl` (even for a war project).
...
[source,options="nowrap"]
----
package com.sankuai.optaplanner.examples.nqueens.solver;
...
